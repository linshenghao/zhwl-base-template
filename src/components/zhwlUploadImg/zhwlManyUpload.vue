...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      setModdleArr(fileList){
        this.isArrNull = window.deepClone(fileList);
      },

      //判断文件是否有变化，有的话才触发提交函数----
      addListenerChange(file, fileList){
        this._changStatus();
        this.setModdleArr(fileList);
        //过滤准备上传的数据
      ...
      beforeUpload(file){
        ...
        //上传之前给每张图片起一个别名
        const keyName = `zhwl-${Number.parseInt(Math.random() * 1000,10) + (new Date().getTime())}.${fileType}`;
