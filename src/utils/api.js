...
axios.interceptors.response.use(data=> {  //响应的拦截
  ...
  if (err.response.status == 504) {
    Message.error({message: '服务器被吃了⊙﹏⊙∥'});
     ...
     Message.error({message: '请求接口不存在呀呀呀⊙﹏⊙∥'});
...
export const postjsonRequest = (url, params,middleware=false) => {
  if(middleware){
    base = "";
  }
  return axios({
